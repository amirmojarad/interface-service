...
// The SearchKeywordFunc type is an adapter to allow the use of ordinary
// function as SearchKeyword mutator.
type SearchKeywordFunc func(context.Context, *ent.SearchKeywordMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SearchKeywordFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.SearchKeywordMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SearchKeywordMutation", m)
	}
	return f(ctx, mv)
}
